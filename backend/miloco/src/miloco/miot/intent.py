...
def resolve_intent(devices: list[dict], request: dict) -> dict:
    ...
    hints: list[str] = []
    executable = [c for c in candidates if not c.get("issue")]
    if not candidates:
        ambiguity = "not_found"
        ...
        ambiguity = "none"
        hints.append(f"命中 1 台：{candidates[0]['name']}（{candidates[0]['room'] or '无房间'}）")
    elif plural:
        ambiguity = "none"
        hints.append(f"命中 {len(candidates)} 台（{'、'.join(_rooms_of(candidates))}），按“全部”处理")
        if len(executable) > MAX_BATCH:
            hints.append(f"超过单次 {MAX_BATCH} 台上限，请分批下发")
    else:
        ambiguity = "multiple"
        names = "、".join(f"{c['name']}({c['room'] or '无房间'})" for c in candidates[:8])
        more = "…" if len(candidates) > 8 else ""
        hints.append(
            f"命中 {len(candidates)} 台：{names}{more}；用户未说“全部”，请反问房间 / 哪一台；"
