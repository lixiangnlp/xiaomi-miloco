...
class SafetySettings(BaseModel):
    ...
    命中 ``protected_categories`` 的设备，agent / CLI / web 发起的控制不直接执行，而是
    stage 成待确认变更，等用户确认后经 ``POST /api/miot/changes/{id}/apply`` 才下发。
    类别名取设备 urn 第 4 段（``urn:miot-spec-v2:device:{category}:…``），与
    ``device list`` 的 category 列一致。
    """
...
class MilocoSettings(BaseSettings):
    ...
    safety: SafetySettings = Field(
        ...
        description="设备控制安全闸门（受保护类别 / 待确认变更有效期 / 规则策略）",
    )
