...
- 受保护类别设备（门锁 / 摄像头 / 可视门铃 / 燃气 / 烟感，`safety.protected_categories`）的控制不直接执行，而是 stage 等待用户确认（见下“危险设备闸门”）；参数由服务端按 spec 校验，越界 / 非法枚举 / 步进不符 / 只读属性返回 422
...
CLI / Agent（miloco-devices Skill）
  ...
  → MiotService.control_device（scope 校验 → 受保护类别判定：命中则 stage 返回，不下发）（miot/service.py）
  → execute_control（服务端值校验 + 下发 + action_ledger，唯一执行核心）（miot/service.py，纯逻辑在 miot/gate.py）
...
规则触发的 STATIC 控制路径不经 HTTP 层，但同样汇入 `execute_control`：`RuleRunner`（`rule/runner.py`）自己做幂等 / 冷却判断，再把动作交给同一个执行核心过闸门、校验、落台账——闸门规则只定义一次，所有路径共用。
...
- **危险设备闸门**：命中 `safety.protected_categories` 的设备不执行、stage 到进程内 `PendingStore`；`apply_change` / `list_changes` / `discard_change` 管理待确认变更
...
原则是“the model stages; a person applies”：模型只能提议，执行由人批准；这条规则在后端强制、apply 时再检查、只定义一次让所有路径共用。

- **判定**：设备类别取 urn 第 4 段（`urn:miot-spec-v2:device:{category}:…`），`miot/gate.py::resolve_protection` 判是否命中 `safety.protected_categories`（默认 `lock` / `camera` / `video-doorbell` / `gas-valve` / `gas-sensor` / `smoke-sensor`）。判定读的是**当前**配置。
- **stage**：`control_device` 对受保护设备先做服务端值校验（错参立刻 422），再登记到进程内 `PendingStore`（dict + TTL，`safety.stage_ttl_sec` 默认 600 秒），返回 `{staged: true, change_id, summary, expires_at, confirm_token, next}`。`confirm_token` 只在这次响应里出现一次，存储侧只留 sha256。
- **apply**：`POST /api/miot/changes/{change_id}/apply` 带 `confirm_token`。token 用常量时间比较；匹配即从存储移出（一次性，不可重放；猜错不销毁变更、返回 403）。随后按当前状态重新把关：scope（设备仍在启用家庭）、spec 值校验，再经同一 `execute_control` 下发——stage 时通过不等于 apply 时通过。
- **list / discard**：`GET /api/miot/changes`（不含 token）、`DELETE /api/miot/changes/{change_id}`。
- **为什么待确认状态放进程内存而不是 action_ledger**：action_ledger 走 `MetricsClient` 异步批量写，写后立即读回不保证可见，不能做“此刻能不能 apply”的权威判定。代价是**后端重启会丢弃未确认的 stage**——600 秒 TTL 下可接受，用户重新发起即可。
- **审计全部落 action_ledger**（schema v5 新增 `status` / `change_id` / `protected` 三列，老行 `status='applied'`、`protected=0`）：stage 写一行 `status=staged`；apply 成功那次真实下发由 `execute_control` 写 `status=applied`，与 stage 行共用同一 `change_id`；token 错误 / apply 复检失败写 `apply_rejected`；过期写 `expired`；用户放弃写 `discarded`；服务端值校验拒绝写 `rejected`。`GET /api/actions?status=…` 与 `miloco-cli actions list --status …` 可按状态过滤。
...
- **当前阶段的边界**：`confirm_token` 随 stage 响应经 CLI 返回，agent 能看到它；本 PR 把“执行”收回服务端并让每一步可审计，但“同意”仍靠 Skill 约束 agent 先问用户。把 token 改投 IM / 面板、让 agent 只拿 `change_id` 是后续工作。
...
| 修改受保护类别判定 / 值校验 / 待确认存储 | `miot/gate.py`（纯逻辑）、`miot/service.py::execute_control`、`config/settings.py::SafetySettings` |
| 修改闸门审计列 / 迁移 | `observability/metrics_db.py`（`_migrate_v5_action_status`）、`observability/types.py::ActionLedgerRecord` |
| 修改 MiOT SDK 封装层  | `miot/client.py`（MiotProxy），更底层看 `backend/miot/src/miot/`                 |
| 修改摄像头管理逻辑    | `miot/camera_handler.py`（`CameraVisionHandler`）                                |

### 设备控制相关 API 路径

主要入口：`POST /api/miot/devices/{did}/control`（控制设备；受保护设备返回 `staged`），`GET /api/miot/changes` / `POST /api/miot/changes/{change_id}/apply` / `DELETE /api/miot/changes/{change_id}`（待确认变更），`GET /api/miot/device_list`（设备列表），完整端点见 `miot/router.py`。
