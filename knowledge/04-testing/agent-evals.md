...
3. **CI 回放录制、比对基线**：live 跑一次得到录制；CI 只对录制重新打分，失败集与 `baseline.json`
...
| `asks_confirmation`                           | `true`：作用域内 reply 命中确认问句（确认、确定、是否、要不要、要我、吗或问号），且没有对 `device_not_controlled` 所列 did 的 control / action（列表为空则要求零 control / action）。`false`：reply 不命中严格确认句（确认、确定要、是否要、要不要、需要我…吗）。 |
